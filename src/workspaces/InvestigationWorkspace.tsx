...
const statusColors: Record<string, { bg: string; text: string; border: string }> = {
  'CONVICTED': { bg: 'rgba(46,204,113,0.12)', text: '#86EFAC', border: 'rgba(46,204,113,0.3)' },
  'CLOSED': { bg: 'rgba(100,116,139,0.12)', text: '#94A3B8', border: 'rgba(100,116,139,0.3)' },
  'UNDER REVIEW': { bg: 'rgba(245,166,35,0.12)', text: '#FCD34D', border: 'rgba(245,166,35,0.3)' },
  'ACTIVE': { bg: 'rgba(240,78,78,0.12)', text: '#FCA5A5', border: 'rgba(240,78,78,0.3)' },
};
...
export function InvestigationWorkspace({
  ...
  return (
    <div className="left-panel-scroll" style={{ flex: 1, overflowY: 'auto', background: 'var(--bg-base, #080C14)', padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 10 }}>

      {/* Mode activated banner */}
      {progress.modeActivated && !progress.profileLoaded && (
        <div className="fade-up" style={{
          background: 'rgba(240,78,78,0.08)', border: '1px solid rgba(240,78,78,0.2)',
          borderRadius: 8, padding: '14px 16px', textAlign: 'center',
        }}>
          <div style={{ fontSize: 14, fontFamily: "'JetBrains Mono', monospace", color: '#FCA5A5', fontWeight: 700, fontVariantNumeric: 'tabular-nums', letterSpacing: '0.12em' }}>
      ...
      {progress.modeActivated && (
        <div className="fade-up" style={{
          ...
          {[
            { label: t('caseConfidence', lang), value: '91%', color: 'var(--accent-blue, #4D9EF5)' },
            { label: t('evidenceNodes', lang), value: `${progress.evidenceNodesShown}/7`, color: 'var(--accent-green, #2ECC71)' },
            { label: t('riskLevel', lang), value: t('high', lang), color: 'var(--risk-high, #F04E4E)' },
            { label: t('associates', lang), value: '7', color: 'var(--accent-purple, #8B6FD4)' },
            { label: t('recommendedAction', lang), value: t('monitorTransactions', lang), color: 'var(--risk-medium, #F5A623)' },
          ].map((item, i) => (
            <div key={i} style={{
              flex: 1, background: 'var(--bg-base, #080C14)', border: '1px solid var(--border-subtle, #1E2D3D)',
              borderRadius: 6, padding: '8px 10px', minWidth: 0,
            }}>
              <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-tertiary, #64748B)', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 4 }}>{item.label}</div>
              <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: item.color, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.value}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {progress.profileLoaded && (
        <div className="fade-up" style={{
          ...
          borderTop: '2px solid rgba(240,78,78,0.5)',
          borderRadius: 10, padding: 20,
          boxShadow: '0 0 40px rgba(240,78,78,0.06)',
        }}>
          {/* Dossier header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, borderBottom: '1px solid #1E2D3D', paddingBottom: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: '#F04E4E', fontWeight: 600, letterSpacing: '0.12em' }}>ACTIVE WARRANT</span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              ...
              {/* Mugshot label */}
              <div style={{
                background: '#131920', border: '1px solid #1E2D3D', borderRadius: 4,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <div style={{
                    ...
                    background: 'rgba(240,78,78,0.15)', border: '1px solid rgba(240,78,78,0.4)',
                    borderRadius: 4, padding: '3px 9px',
                  }}>
                    <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#F04E4E', display: 'inline-block' }} />
                    <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#FCA5A5', fontWeight: 700, fontVariantNumeric: 'tabular-nums', letterSpacing: '0.08em' }}>{t('highRisk', lang)}</span>
                  </div>
                  <div style={{ background: '#131920', border: '1px solid #243447', borderRadius: 4, padding: '3px 9px' }}>
                    <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#8B6FD4', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>CLUSTER K-7</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: '#64748B', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 5 }}>{t('connectedCrimes', lang)}</div>
                ...
                <div>
                  <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: '#64748B', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 5 }}>{t('linkedCases', lang)}</div>
                  <div style={{ display: 'flex', gap: 5 }}>
                    {[{ id: 'KS1207', crime: 'Money laundering' }, { id: 'KS1189', crime: 'Drug trafficking' }].map(c => (
                      <div key={c.id} onClick={() => onCaseClick(c.id)}
                        style={{
                          background: 'var(--bg-raised, #131920)', border: '1px solid var(--border-default, #243447)', borderRadius: 5,
                          padding: '5px 9px', cursor: 'pointer',
                        }}>
                        <div style={{ fontSize: 12, fontFamily: 'var(--font-mono)', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: 'var(--accent-blue, #4D9EF5)', letterSpacing: '0.04em' }}>{c.id}</div>
                        <div style={{ fontSize: 10, color: 'var(--text-tertiary, #64748B)' }}>{c.crime}</div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {progress.casesLoaded && (
        <div className="fade-up" style={{ background: 'var(--bg-raised, #131920)', border: '1px solid var(--border-subtle, #1E2D3D)', borderRadius: 8, padding: '12px 14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#94A3B8', fontWeight: 600, letterSpacing: '0.08em' }}>{t('detentionTimeline', lang)}</span>
            <span style={{ fontSize: 12, fontFamily: "'JetBrains Mono', monospace", color: '#FCA5A5' }}>{t('imprisonedLabel', lang)}</span>
          </div>
          <div style={{ background: '#1E2D3D', borderRadius: 4, height: 8, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', left: 0, top: 0, width: '40%', height: '100%', background: 'rgba(240,78,78,0.7)', borderRadius: '4px 0 0 4px' }} />
      ...
      {progress.casesLoaded && (
        <div className="fade-up" style={{ background: 'var(--bg-raised, #131920)', border: '1px solid var(--border-subtle, #1E2D3D)', borderRadius: 8, padding: '12px 14px' }}>
          <div style={{ fontSize: 13, fontFamily: "'JetBrains Mono', monospace", color: '#94A3B8', fontWeight: 600, letterSpacing: '0.12em', marginBottom: 10 }}>
            ...
            {OFFENDER_FOOTPRINT.map((fp) => {
              ...
              return (
                <div key={fp.caseId} onClick={() => onCaseClick(fp.caseId)}
                  style={{
                    background: '#0D1117', border: '1px solid #243447', borderRadius: 6,
                    padding: '10px 14px', minWidth: 150, cursor: 'pointer',
                    transition: 'border-color 0.15s',
                    flexShrink: 0,
                  }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = '#4A5C70')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = '#243447')}
                >
                  <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: 'var(--accent-blue, #4D9EF5)', marginBottom: 3, letterSpacing: '0.04em' }}>{fp.caseId}</div>
                  ...
                  <div style={{ background: sc.bg, border: `1px solid ${sc.border}`, borderRadius: 4, padding: '2px 6px', display: 'inline-block' }}>
                    <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: sc.text }}>{fp.status}</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="fade-up" style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, fontVariantNumeric: 'tabular-nums', letterSpacing: '0.12em', color: 'rgba(220,38,38,0.85)', fontFamily: "'JetBrains Mono', monospace" }}>
                ...
                return (
                  <div key={ev.id} style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    >
                      ...
                      {/* Type label */}
                      <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: typeColor, fontWeight: 600, letterSpacing: '0.12em', marginBottom: 6 }}>{ev.type}</div>
                      <Icon size={17} color={typeColor} />
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#E8EDF2', marginTop: 6, textAlign: 'center' }}>{ev.title}</div>
                      ...
                      <div style={{ width: '100%', marginTop: 10 }}>
                        ...
                        <div style={{ textAlign: 'right', fontSize: 11, fontFamily: 'var(--font-mono)', fontVariantNumeric: 'tabular-nums', color: typeColor, marginTop: 3 }}>
                          {ev.confidence}%
                        </div>
                      </div>
                    </div>
              ...
              {progress.evidenceNodesShown >= 6 && (
                <>
                  ...
                  <div style={{
                    ...
                    background: 'rgba(240,78,78,0.1)', border: '2px solid rgba(240,78,78,0.5)',
                    boxShadow: '0 0 24px rgba(240,78,78,0.18)',
      ...
      {progress.networkLoaded && (
        <div className="fade-up" style={{
          background: 'var(--bg-raised, #131920)', border: '1px solid var(--border-subtle, #1E2D3D)', borderRadius: 8, padding: '14px 16px',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Users size={14} color="#8B6FD4" />
              <span style={{ fontSize: 13, fontWeight: 700, fontVariantNumeric: 'tabular-nums', letterSpacing: '0.12em', color: '#94A3B8', fontFamily: "'JetBrains Mono', monospace" }}>
                {t('gangStructure', lang)} · Cluster K-7
              </span>
            </div>
            <span style={{
              background: 'rgba(240,78,78,0.1)', border: '1px solid rgba(240,78,78,0.2)',
              borderRadius: 4, padding: '2px 8px',
              fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#FCA5A5',
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <div style={{ width: 1, height: 20, background: 'rgba(240,78,78,0.3)' }} />
          </div>

          {/* Horizontal line for level 1 */}
          <div style={{ position: 'relative', marginBottom: 0 }}>
            <div style={{ height: 1, background: 'rgba(240,78,78,0.25)' }} />
            {/* Drops */}
            <div style={{ display: 'flex', justifyContent: 'space-around' }}>
              {level1.map(() => (
                <div key={Math.random()} style={{ width: 1, height: 10, background: 'rgba(240,78,78,0.25)' }} />
              ))}
            </div>
          </div>
          ...
          <div style={{ height: 1, background: 'rgba(240,78,78,0.15)', margin: '8px 0 0' }} />
          <div style={{ display: 'flex', justifyContent: 'space-around' }}>
            {level2.map(() => (
              <div key={Math.random()} style={{ width: 1, height: 10, background: 'rgba(240,78,78,0.15)' }} />
            ))}
          </div>
          ...
          <div style={{ textAlign: 'center', fontSize: 10, color: '#4A5C70', marginTop: 4 }}>
            {t('clickForFile', lang)}
          </div>
        </div>
      )}
      ...
      {progress.recommendationLoaded && (
        <div className="fade-up" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#FCD34D', letterSpacing: '0.12em', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
              {t('aiRecommendation', lang)}
            </span>
            <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: '#64748B' }}>· 91% confidence</span>
          </div>
...
function GangCard({ member, size, onClick, lang }: {
  ...
  return (
    <div className="gang-card" onClick={onClick} style={{
      background: isTarget ? 'rgba(245,166,35,0.08)' : '#131920',
      border: `1px solid ${isTarget ? 'rgba(245,166,35,0.45)' : '#1E2D3D'}`,
      boxShadow: isTarget ? '0 0 18px rgba(245,166,35,0.15)' : undefined,
      ...
      {isTarget && (
        <div style={{
          ...
          background: '#F5A623', color: '#000', fontSize: 8, fontWeight: 800,
      ...
      <div style={{
        ...
        padding: '2px 5px', fontSize: 9, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: sc.text,
      }}>{member.status}</div>
    </div>
  );
}
